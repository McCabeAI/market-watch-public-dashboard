...
Load the current canonical paper books from `data/overnight/books/latest.json` and give every advocate the same competition context plus the compact deterministic `funding_context`: latest official NY Fed SOFR, tradable SR3 contracts, and exact-horizon forward summaries when an SR3 expiry month matches. The competition metric is cumulative **net paper P&L after financing economics**. Every seat starts with $100m paper NAV and earns the common official SOFR ACT/360 cash hurdle on that NAV; any risk it takes is charged the same SOFR on trusted **1%-shock risk capital**, regardless of asset class. The trusted per-seat risk-capital ceiling is $1m and the hard high-water-mark drawdown stop is $5m. Notional is descriptive. There is no fixed 5% assumption and no special financing subsidy for the no-trade skeptic: flat simply means zero shocked risk capital. This is context and incentive, not evidence for a macro thesis. A model must never calculate or overwrite canonical P&L, funding, NAV, risk capital, drawdown state, or rank.
