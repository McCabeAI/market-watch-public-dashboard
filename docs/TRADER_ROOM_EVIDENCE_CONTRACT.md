...
The same market-state object carries country housing blocks for the US, Canada and Australia. `us_housing` covers FHFA/Census/Fed/Freddie Mac prices, new-home sales/inventory, construction, mortgage credit, rates and mortgage debt-service burden. `canada_housing` covers Statistics Canada/CMHC/Bank of Canada new-home prices, construction/permits, mortgage lending/balances/rates and mortgage debt-service burden. `australia_housing` covers ABS/RBA prices/turnover, approvals, lending, credit, rates and cash-flow burden. These are common packet evidence for every advocate, not private follow-up research. Missing/stale housing sub-feeds retain their source status and must not be silently replaced.
...
Load the current canonical paper books from `data/overnight/books/latest.json` and give every advocate the same competition context plus the compact deterministic `funding_context`: latest official NY Fed SOFR, tradable SR3 contracts, and exact-horizon forward summaries when an SR3 expiry month matches. The competition metric is cumulative **net paper P&L after financing economics**. Official NY Fed SOFR ACT/360 is the zero-return benchmark: paper NAV may be described as earning SOFR, but it is equally funded/benchmarked at SOFR so those flows cancel. Any deployed risk is charged the same SOFR on trusted **standard-shock risk capital**, regardless of asset class. A flat book, including no-trade-skeptic, has zero net financing P&L. The trusted per-seat risk-capital ceiling is $10m and the hard high-water-mark drawdown stop is $5m. Notional is descriptive. There is no fixed 5% assumption and no special cash-manager subsidy. This is context and incentive, not evidence for a macro thesis. A model must never calculate or overwrite canonical P&L, funding, NAV, risk capital, drawdown state, or rank.
