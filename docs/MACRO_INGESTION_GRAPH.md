...
- Unit tests no longer refresh live-smoke fixtures or write `data/macro_ingestion/post_freeze/` unless `MACRO_INGESTION_LIVE_SMOKE=1` (smoke) or the CLI observations directory is the repo default (post-freeze). The committed smoke reports are the session evidence.
