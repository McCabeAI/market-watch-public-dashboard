...
Grinder keeps the highest deployment hurdle, but it may not convert packet imperfections into a blanket NO_TRADE. Every automated Grinder decision must include a validated `deployment_hurdle` section. It must evaluate the best independent **markable** frozen handoff candidates against official SOFR on shocked-risk capital and identify missing data at the candidate level. When at least two markable handoff opportunities exist, at least two must be assessed. A candidate may be `not_evaluable` only when the section names decision-critical missing data specific to that instrument and explains why price/risk/carry/catalyst/edge cannot be judged without it. Unrelated gaps are ignored, not accumulated into a global veto. The packet being an on-demand fallback is not itself sufficient reason to stay flat. Forward points/carry, when available later, enter the FX edge calculation; their absence does not block unrelated candidates whose expected edge can otherwise be evaluated.
