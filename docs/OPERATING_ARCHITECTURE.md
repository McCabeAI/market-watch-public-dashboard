...
Known operational caveat: a merge performed by the overnight acceptance workflow with its GitHub Actions token does not itself enqueue the push-triggered Pages workflow. Until the acceptance workflow explicitly chains a Pages dispatch, an accepted same-day rerun may require a separate deploy trigger even though canonical books on `main` are already current.
