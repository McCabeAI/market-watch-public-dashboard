...
14. The deploy job publishes that artifact to GitHub Pages on ordinary `main` pushes, manual dispatch, the weekday 04:15 America/New_York schedule, and a `workflow_dispatch` from the overnight acceptance workflow after a confirmed accepted-output merge. A merge made with that workflow's GitHub Actions token does not itself enqueue the push trigger, so the explicit dispatch is what publishes newly accepted trader and PM books. Validation, review-identity, canonical-book, or apply failure does not dispatch Pages. The 04:07 overnight gate runs first and scheduled Pages publication requires the assembled morning dataset. GitHub Actions remains the only website publisher.
