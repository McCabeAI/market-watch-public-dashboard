...
Trusted overnight code owns deterministic paper mids, a **$1m 1%-shock risk-capital ceiling** per $100m-paper-NAV seat, a **$5m high-water-mark drawdown stop**, ledger linkage, and persistence to `data/overnight/books/latest.json`. Notional is descriptive rather than a sizing ceiling. Risk capital is the absolute MTM loss from a 1% adverse move in the quoted risk factor, with a 1bp minimum shock for rates/spreads. Mixed lists may combine de-risk actions with blocked expansions; valid `REDUCE` / `CLOSE` still apply when an over-risk-cap `OPEN` / `ADD` / `HEDGE` is blocked. A drawdown breach is trusted-code state: the book is forcibly flattened at deterministic marks and new OPEN/ADD/HEDGE is blocked while `risk_stopped`.
