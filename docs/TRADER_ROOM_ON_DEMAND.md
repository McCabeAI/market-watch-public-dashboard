...
Trusted overnight code owns deterministic paper mids, a **$10m standard-shock risk-capital ceiling** per $100m-paper-NAV seat, a **$5m high-water-mark drawdown stop**, ledger linkage, and persistence to `data/overnight/books/latest.json`. Notional is descriptive rather than a sizing ceiling. Risk capital is the absolute MTM loss from the standard adverse move: 1% in spot FX, 100bp in outright rates, or 100bp in a curve/RV spread. Mixed lists may combine de-risk actions with blocked expansions; valid `REDUCE` / `CLOSE` still apply when an over-risk-cap `OPEN` / `ADD` / `HEDGE` is blocked. A drawdown breach is trusted-code state: the book is forcibly flattened at deterministic marks and new OPEN/ADD/HEDGE is blocked while `risk_stopped`.
