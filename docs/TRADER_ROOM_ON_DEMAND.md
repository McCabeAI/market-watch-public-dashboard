...
- Linear spreads/flies may be built from packet rate legs. Forward swaps/fwd-fwds may use `paper_expression.type = "forward_swap_proxy"` with `country`, `start_years`, `tenor_years` and optional `payment_frequency`. Trusted code computes `(P_start - P_end) / sum(alpha_i * P_i)`.
- For the paper competition, the official Fed/BoC/RBA government zero/forward curves are deliberately accepted as close-enough proxies for swap/OIS forward-forward marks. Preserve that provenance; do not block a trade merely because the curve is government rather than OIS.
- A trader may therefore express, for example, `pay US 2y2y` as `{"type":"forward_swap_proxy","country":"US","start_years":2,"tenor_years":2}`. The book recomputes the same proxy mid on each review.
