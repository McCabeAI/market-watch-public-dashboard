...
| Swinger / Pragmatist / Grinder PMs | `grok-4.6`, one call each, no subagents |
...
- three independent PMs = 3 Grok 4.6
- **Total Grok ceiling including parent = 32**
- **Composer ceiling = 28**
- **Total model-invocation ceiling = 60**
...
- `pm_decisions.json` containing independent decisions for Swinger, Pragmatist and Grinder

## PM layer

The Trader Room is research/trade generation. Risk ownership sits one layer above it.

- Four independent PM books exist: ChatGPT PM, Swinger, Pragmatist and Grinder.
- Each book has a hard $1bn gross-notional ceiling and uses the same deterministic paper marks/P&L engine.
- All four receive the same evidence and trader work; only their objective differs.
- The three model PMs commit independently after the 14-seat debate and may not inspect each other's current decisions.
- Swinger may not use HEDGE; it must reduce or close instead.
- ChatGPT PM is not synthesized by Cursor. It writes through the separate PM decision ingest contract after ChatGPT arbitration.
- PM models/chat author decisions only; trusted code owns entry/exit marks, locked curve construction and P&L.
