"""00:07 ET collect: snapshot current hard inputs without rewriting the dashboard."""
...
NEWS_FILES = (
    ...
    "scripts/apply_daily_20260918.py",
...
def _file_digest(root: Path, rel: str) -> dict[str, Any]:
    ...
    return {"path": rel, "status": "present", "digest": sha256_file(path), "bytes": path.stat().st_size}
...
def _family(status: str, *, as_of: str | None, digest: str | None, notes: list[str], extra: dict[str, Any] | None = None) -> dict[str, Any]:
...
def collect_inputs(
    ...
    *,
    ...
    except Exception as exc:  # noqa: BLE001 - fail loud into family status
        macro_status = "invalid"
        macro_notes.append(f"temperature_scores invalid: {exc}")
    families["macro_hard"] = _family(macro_status, as_of=macro_as_of, digest=macro_digest, notes=macro_notes, extra={"files": macro_files})

    news_digest, news_files = _concat_digest(root, NEWS_FILES)
    news_as_of = _parse_news_as_of(root)
    if news_digest is None:
        news_status = "invalid"
        news_notes = ["required news patch files missing or unreadable"]
        ...
        news_notes = ["news family is the current dashboard patch chain; collect does not rewrite it"]
    families["news"] = _family(news_status, as_of=news_as_of, digest=news_digest, notes=news_notes, extra={"files": news_files})

    research_digest, research_files = _concat_digest(root, RESEARCH_FILES)
    if research_digest is None:
        research_status = "missing"
        research_as_of = None
        research_notes = ["central-bank research transform files missing"]
    else:
        research_as_of = news_as_of
        research_status = age_status(research_as_of, when=stamp) if research_as_of else "stale"
        research_notes = ["research family snapshots the current rolling 30-day transform, not a live scrape"]
    families["central_bank_research"] = _family(
        research_status, as_of=research_as_of, digest=research_digest, notes=research_notes, extra={"files": research_files}
    )
    ...
    if market_state_path and Path(market_state_path).is_file():
        ...
        market_as_of = None
        market_digest = None
        market_notes.append("offline collect: market_state not fetched; not catastrophic")
    else:
        try:
    ...
    families["market_state"] = _family(market_status, as_of=market_as_of, digest=market_digest, notes=market_notes)
    ...
    snapshot = {
        ...
        "families": families,
        "temperature_scores": {
            "last_refresh_date": None if scores is None else scores.get("last_refresh_date"),
            "digest": None if scores is None else sha256_json(scores),
        },
        "preservation": {
