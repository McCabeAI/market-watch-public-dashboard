...
Notional is descriptive, not the risk budget. Each PM has $1bn paper NAV, a trusted **$100m standard-shock risk-capital limit**, and a trusted **$50m high-water-mark drawdown stop**. Official SOFR ACT/360 is the zero-return benchmark and is charged on shocked risk capital; equal risk capital receives equal financing treatment across asset classes. Flat cash at SOFR is not alpha. Size from plausible adverse paths and historical drawdowns, not as a percentage of a gross-notional cap and never merely to fill available capacity. If trusted state is `risk_stopped`, do not submit OPEN/ADD/HEDGE; the book remains flat until the stop is explicitly reset by trusted policy.

Pragmatist must include a validated `portfolio_construction` section before final actions: existing book, independent markable Trader Room opportunities, the main adverse scenario, whether candidates complement/diversify/offset or duplicate beta, and reasons for adding or rejecting complementary trades. When the frozen packet contains independent markable handoff alternatives, those instruments must actually be enumerated in `independent_handoff_opportunities` (at least two when two or more exist). Listed rows must be packet/handoff instruments, not invented names. One-position and flat outcomes remain valid after that evaluation. Swinger may still concentrate. ChatGPT remains unconstrained.

Grinder may still stay flat, but every decision must include validated `deployment_hurdle`. Missing data is candidate-specific, not a global veto. Evaluate the best markable handoff opportunities against official SOFR on shocked-risk capital; when two or more markable candidates exist, assess at least two. `not_evaluable` requires explicit decision-critical missing data for that specific instrument. Unrelated gaps must be ignored and cannot justify NO_TRADE. A fallback packet label alone is not a deployment blocker. Future forward points/carry belong in the FX hurdle when available, but their current absence does not block unrelated candidates.
