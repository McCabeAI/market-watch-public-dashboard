...
Paper execution is at deterministic packet mid. A broker-executable quote is not required for the paper book. Direct instruments use the frozen reference/fixing/settlement/yield mid. Derived rates structures are allowed. The packet's official Fed/BoC/RBA government zero/forward curves are intentionally accepted as close-enough paper proxies for swap/OIS fwd-fwds. Use `paper_expression.type="forward_swap_proxy"` with country/start_years/tenor_years (for example US 2y2y) and let trusted code compute/recompute the mid from discount factors. Do not reject the structure solely because the proxy is government rather than OIS. Advocates may use their permitted Composer subagents to analyze curve construction, but canonical paper marks/P&L are deterministic. If the live run records a separate `paper_capital`/paper-action object, copy the exact same `paper_expression` into that object so the deterministic book can preserve and re-mark the structure; never collapse a derived trade into an outright surrogate.
