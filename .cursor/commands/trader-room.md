...
Paper execution is at deterministic packet mid. A broker-executable quote is not required for the paper book. Direct instruments use the frozen reference/fixing/settlement/yield mid. Derived rates structures are allowed with a structured `paper_expression`. The packet's official US/Canada/Australia government zero/forward curves are the accepted close-enough paper proxy for swap/OIS curve expressions. Linear spreads/flies and fwd-fwds are valid; a forward swap uses `(P_start - P_end) / sum(alpha_i * P_i)`. Prefer compact syntax such as `{"type":"forward_swap","curve_country":"CA","start_years":2,"tenor_years":2,"payment_frequency":1}` for CA 2y2y. Trusted code resolves and re-marks the proxy; models do not author canonical arithmetic. Advocates may use their permitted Composer subagents to analyze curve construction.
